    ...
    <div className="bg-bg-image  py-40 text-center text-2xl  sm:py-4 xs:py-20 xs:font-medium">
      <div className="bg-teal-500 py-40 mx-64 mb-5   sm:mx-20 sm:my-5 xs:grid-rows-4 xs:mx-8 xs:my-0 xs:py-8">
        ...
        <input
          ...
          className=" px-5 py-3 sm:mr-1 sm:px-0 sm:py-0  xs:py-0 xs:px-0.5  xs:mr-20 xs:mb-3 "
          value={task}
        ...
        <div>
          {list.map((ele, index) => (
            <div
              key={index}
              className="bg-teal-300 mx-10 mt-6 py-3 space-y-10 xs:py-0 "
            >
              <div>
                <span className="font-bold text-2xl">{ele.task}</span>
                <span className="ml-40 font-serif font-bold text-2xl xs:ml-0">
                  {ele.time}
                </span>
                <button
                  className="ml-40 text-2xl xs:ml-20"
        ...
        {list.length >= 1 && (
          <button
            className="bg-teal-100 mt-10 px-4 py-2 hover:bg-stone-200 xs:py-1"
